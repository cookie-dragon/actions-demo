...
def check_block(three_sha256, r_three_sha256):
    return True if three_sha256[0] == r_three_sha256[0] else False, \
           ...
           True if three_sha256[3] == r_three_sha256[3] else False


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Mount Tools')
    parser.add_argument("--cb", type=str, default="check")
    parser.add_argument("--auth", type=int,
                        default=2)  # 0:No Auth;1:Check Card Only;2:Check BOXID;3:Check Serial;4:Check All
    args = parser.parse_args()

    three_sha256 = get_three_sha256(get_serial(), get_cid(), get_boxid())

    exit_sys = 1
    if args.cb == "build":
        exit_sys = write_block(three_sha256)

    elif args.cb == "check":
        if args.auth == 0:
            exit_sys = 0
        else:
            rst = check_block(three_sha256, read_block())
            if rst[3]:
