...
class WlanInterface(NetworkInterface):
    ...
    def config(self):
        ...
        if self.mode == "main" or self.mode == "vice":
            os.system('echo "ctrl_interface=/var/run/wpa_supplicant" > /etc/wpa_supplicant.conf')
            if self.station_psk != "":
                os.system(
                ...
                os.system('sed -i "5c key_mgmt=NONE" /etc/wpa_supplicant.conf')
                os.system("sync")
            if self.station_inet == "static":
    ...
    def start(self):
        if self.mode == "main" or self.mode == "vice":
            ...
            if self.station_inet == "static":
                ...
                os.system('/usr/local/sbin/wpa_cli -a/opt/bin/wpa_autodhcpc -B')
        elif self.mode == "gateway":
    ...
    def stop(self):
        if self.mode == "main" or self.mode == "vice":
            ...
            if self.station_inet == "dhcp":
                JobShell.killjob('udhcpc -i ' + self.get_name())
